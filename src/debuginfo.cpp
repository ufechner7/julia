...
void RTDyldMemoryManagerUnix::registerEHFrames(uint8_t *Addr,
  ...
  size_t cur_entry = 0;
  processFDEs((char*)Addr, Size, [&](const char *Entry, unsigned encoding){
      uintptr_t *EntryPtr = ((uintptr_t *)Entry)+1;
      ...
      // I'm not entirely sure this is correct, but
      // it seems to be fine as long as it's consisten
      table[cur_entry].start_ip_offset =
        safe_trunc<int32_t>((intptr_t)start - (intptr_t)Addr);
